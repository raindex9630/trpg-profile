...
- 通常時は閲覧用と同じ月間カレンダーが全面表示される。
- 年月・月移動ボタン・凡例は中央3段、最終更新日時は右上に表示される。PCの文字サイズ、種別色、土日祝の色、枠と月メモの寸法は公開閲覧画面に合わせている。
- 既存の data/calendar.json の予定が見える。
- カレンダー上でホイールを1段回すごとに、待ち時間なく1か月ずつ連続して移動する。大きく回した入力では入力量に応じて複数月を移動する。
- 「＋ セッション追加」で左編集パネルが開き、カレンダーが右へ縮小される。
- 新規追加で最後の日付の選択を外すと左編集パネルが閉じる。名前・時刻などを入力済みの場合は破棄確認が表示され、キャンセルすると日付と入力内容が維持される。
- 小さい画面では編集パネルとカレンダーが縦に並び、横スクロールが出ない。
- 月メモへポインターまたはキーボードフォーカスを合わせ、「月メモを編集」を押す。タッチ環境では編集ボタンが常に表示される。
- 複数日選択、日程ごとの時間編集、キャンセル、Undo、Redo、JSON退避を試せる。
...
1. カレンダー下の「月メモを編集」を押す。
2. 月メモを少し変更する。
3. 「月メモを保存」を押す。
4. 「GitHubへ保存しました」と表示されることを確認する。
5. GitHubで trpg-profile/data/calendar.json を開く。
6. 新しいコミットが作られ、updated_atが日本時間で更新されたことを確認する。
7. 既存の公開PagesプロジェクトのDeploymentsを開く。
8. mainのpushによる新しいデプロイが成功するまで待つ。
9. 公開サイト https://trpg-profile.pages.dev/calendar.html を再読込し、変更を確認する。
...
左編集パネルの「登録」「変更を保存」、月メモの「月メモを保存」、ページ設定の「ページ設定を保存」は、入力内容をGitHubへ直接保存します。成功メッセージを確認して操作を完了します。

閉じる、キャンセル、Escは、入力内容が変わっている場合に破棄確認を表示します。通信失敗や競合が発生した場合は入力内容を画面へ保持します。

### セッションと日程の違い

- セッション情報: シナリオ名、陣、種別。同じセッションに属するすべての日程へ共通。
- 日程情報: 日付、時間、予備日、補足、終日・昼・夜。選択した日程だけに適用。

左パネルの日程欄で時間、予備日、補足を変更すると、その行の日程だけへ適用されます。カレンダーの日付または予定カードから左パネルを開くと、クリックした日付の日程欄へ即座に移動し、対象の日付セルと日程カードを濃い枠と背景色で強調します。同じセッションの別日をカレンダーから押すと、強調と表示位置がその日へ切り替わります。「全日程に適用」を押すと、入力した時刻または時間帯を表示中の全日程へ適用します。既存セッションへ新しい日程を選ぶと「追加した日程だけに適用」が現れ、既存日程の時刻を維持したまま、未保存の追加日程だけへ一括時刻を設定できます。

### 新しいセッションを作る

1. カレンダー上部の「＋ セッション追加」を押す。
2. 左パネルでシナリオ名、陣、種別を入力する。
3. カレンダーで1日以上の日付を選ぶ。
4. 左パネルの日程ごとに時間、予備日、補足を設定する。
5. 「登録」を押す。
6. GitHubへの保存成功メッセージを確認する。

### 既存セッションへ日程を追加する

1. カレンダーの対象予定カードを押す。
2. 左パネルの「＋ このセッションに日程を追加」を押す。
3. カレンダーで追加日を1日以上選ぶ。
4. 追加日を同じ時刻にする場合は「一括時刻」へ開始・終了時刻を入力し、「追加した日程だけに適用」を押す。
5. 必要に応じて、左パネルの日程ごとに予備日・補足などを設定する。
6. 「変更を保存」を押す。
7. GitHubへの保存成功メッセージを確認する。
...
3. 日付、時間、予備日、補足を変更する。
4. 「変更を保存」を押す。
5. GitHubへの保存成功メッセージを確認する。

### 日程を移動・削除する

1. カレンダー内の対象予定カードを押す。
2. 左パネルの日程行で日付を変更する。
3. 1日程を削除する場合は、その日程行の「×」を押す。
4. セッション全体を削除する場合は、左下の「セッション削除」を押して確認する。
5. 日程の変更後は「変更を保存」を押す。
