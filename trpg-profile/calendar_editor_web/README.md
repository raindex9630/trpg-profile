...
左パネルの日程欄で時間、予備日、補足を変更すると、その行の日程だけへ適用されます。カレンダーの日付または予定カードから左パネルを開くと、クリックした日付の日程欄へ即座に移動します。「全日程に適用」を押すと、入力した時刻または時間帯を表示中の全日程へ適用します。既存セッションへ新しい日程を選ぶと「追加した日程だけに適用」が現れ、既存日程の時刻を維持したまま、未保存の追加日程だけへ一括時刻を設定できます。
